...
function WPSP_GetOptions(){
	...
	$options = array();
	// Slideshow Home
		$sqlHome = "SELECT option_value FROM ".$wpdb->prefix."options WHERE option_name = '".$wpdb->prefix."slideshow_posts_options' " ;
		// Slideshow Noticias
		$sqlNoticias = "SELECT option_value FROM ".$wpdb->prefix."options WHERE option_name = '".$wpdb->prefix."slideshow_posts_options_noticias' " ;
		// print($_SERVER["REQUEST_URI"]);
		if (is_numeric(strpos($_SERVER["REQUEST_URI"], "/noticias/"))) {
			$suboptions = explode("~",$wpdb->get_var($sqlNoticias));
		}else{
			$suboptions = explode("~",$wpdb->get_var($sqlHome));
		}
...
// Plugin modificado para darle mis opciones personalizadas, el objetivo era poder elegir la categoria.
