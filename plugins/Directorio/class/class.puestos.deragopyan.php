<?php 
	...
	*/
	class D_Puestos
	{
		...
		private static $option_name = "puesto";
		private static $optionTableCount = "dir_puesto";
		...
		public static function newOption($data){
			if ($data["directorio"] == 1):
				self::$datos = $data;
				self::insert();
				self::$success = "Puesto creado correctamente";
			endif;
		}
	...
	add_action( 'wp_ajax_puestos', 'puestos' );
	function puestos(){
		if($_POST["action"] == "puestos"):
			if($_POST["actionOption"] == "edit"):
				D_Puestos::edit($_POST["id"],$_POST["value"]);
			elseif($_POST["actionOption"] == "delete"):
				D_Puestos::dissappear($_POST["id"]);
			endif;
		endif;
		die();
	}
